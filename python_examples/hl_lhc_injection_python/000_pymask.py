...
i_oct_f = configuration['oct_current_f']
i_oct_d = configuration['oct_current_d']
beam_str = {'lhcb1':'b1', 'lhcb2':'b2'}[sequence_to_track]
for ss in '12 23 34 45 56 67 78 81'.split():
   mad_track.input(f'kof.a{ss}{beam_str} = kmax_mo*({i_oct_f})/imax_mo/({brho});')
   mad_track.input(f'kod.a{ss}{beam_str} = kmax_mo*({i_oct_d})/imax_mo/({brho});')
